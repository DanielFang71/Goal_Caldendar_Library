...
function Calendar(id, size, labelSettings, colors, options) {
  ...
  for (var i = 0; i < 7; i++) {
    this.labels.push(this.label[i].substring(0, labelSettings[1]));
  }
...
Calendar.prototype.addData = function (objs) {
  let data = {};
...
Calendar.prototype.createGoal = function (complete, goal, text) {
  return { complete: complete, goal: goal, text: text };
};
Calendar.prototype.addGoalToObjs = function (date, goal) {
  this.goalsObj.push({ date: date, goal: goal });
  return this.goalsObj;
};

Calendar.prototype.changeColor = function (newColors) {
  this.colors = newColors;
};

Calendar.prototype.changeSize = function (newSize) {
  this.size = newSize;
};

Calendar.prototype.changeLableSetting = function (newLabelSettings) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    span.appendChild(
      document.createTextNode(
        "Finish: " + data[i].complete + " / " + data[i].goal
      )
    );
